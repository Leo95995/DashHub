...
const CryptoGainersAndLosers: React.FC = () => {
  ...
  if (error) {
    return <> <ErrorMessage message={"Error while fetching crypto gainers & losers datas"}/> </>;
  }
        ...
        {data?.map((coin: ICryptoTopGainers) => {
          return (
            <>
              <div
                key={coin.id}
                className="flex items-center cursor-pointer hover:rounded-md mx-4 hover:bg-gradient-to-r from-gray-50 to-gray-500 hover:scale-105 hover:text-black gap-3 p-2 border dark:border-gray-600 border-gray-200 rounded shadow-sm hover:shadow-md transition"
              >
                <img
                  src={coin.image}
